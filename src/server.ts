...
const API_PROXY_PREFIX = '/transcript-api/';
...
function proxy(req: IncomingMessage, res: ServerResponse, tail: string, search: string): void {
  ...
  const upstream = https.request({
    ...
    // Do not forward upstream cookies into the ScriptFlow origin. The preview is
    // intentionally stateless and never exposes Transcript Studio cookies to CRM code.
...
function proxyTranscriptApi(req: IncomingMessage, res: ServerResponse, tail: string, search: string): void {
  let target: URL;
  try {
    target = new URLCtor('/api/' + tail.replace(/^\/+/, '') + search, TARGET);
    if (target.origin !== TARGET.origin) throw new Error('Target not allowed');
  } catch {
    res.writeHead(400, { 'Content-Type': 'application/json; charset=utf-8' });
    res.end(JSON.stringify({ error: 'Invalid Transcript Studio API URL' }));
    return;
  }
  const headers: Record<string, string | string[] | undefined> = { ...req.headers };
  headers.host = TARGET.host;
  ...
  upstream.on('timeout', () => upstream.destroy(new Error('Transcript Studio API timed out')));
  upstream.on('error', (error: Error) => {
    if (res.headersSent) { res.destroy(error); return; }
    res.writeHead(502, { 'Content-Type': 'application/json; charset=utf-8' });
    res.end(JSON.stringify({ error: 'Transcript Studio API proxy failed', message: error.message }));
  });
  req.pipe(upstream);
}
...
const server = http.createServer((req, res) => {
  try {
    ...
    if (pathname.startsWith(API_PROXY_PREFIX)) {
      return proxyTranscriptApi(req, res, pathname.slice(API_PROXY_PREFIX.length), requestUrl.search);
    }
    ...
    if (pathname.startsWith(PROXY_PREFIX)) {
      if (req.method !== 'GET' && req.method !== 'HEAD') {
        ...
        res.end(JSON.stringify({ error: 'In-app Transcript Studio preview supports GET/HEAD navigation only.' }));
        return;
      }
