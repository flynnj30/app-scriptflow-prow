...
await fs.mkdir(path.join(dist, 'transcript-studio'), { recursive: true });
execFileSync(npx, ['vite', 'build', '-c', 'vite.transcript.config.ts'], { stdio: 'inherit' });
console.log(`Production build complete: ${clientFiles.length} CRM browser modules compiled and minified, plus embedded Transcript Studio.`);
