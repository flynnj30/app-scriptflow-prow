...
function openSmartImportEnhanced() {
    ...
    if (!modal) {
        ...
        showToast('Smart Import modal not found', 'error');
        return;
    }
...
async function parseAndPreviewImportEnhanced() {
    ...
    try {
        ...
        if (aiAvailable && SmartImportState.useAI && typeof geminiService !== 'undefined') {
            // Use Gemini AI
...
// Export SmartImportDOM for debugging
window.SmartImportDOM = SmartImportDOM;

// Also expose to window with consistent naming
window.SmartImport = {
...
console.log('🔄 Functions exposed globally:', Object.keys(window).filter(k => k.includes('Import') || k.includes('smart')).join(', '));