...
const ErrorHandler = {
    ...
    loadErrorLog() {
        try {
            ...
            if (stored) {
                ...
                if (!Array.isArray(this.config.errorLog)) {
    ...
    showErrorPage(error) {
        ...
        errorPage.innerHTML = `
            <div style="max-width: 520px; width: 100%; text-align: center; padding: 32px; background: var(--bg-secondary, #1e293b); border-radius: 24px; border: 1px solid var(--border-color, #334155); box-shadow: var(--shadow-lg, 0 20px 35px -8px rgba(0,0,0,0.4));">
                <div style="font-size: 4rem; margin-bottom: 16px;">⚠️</div>
                <h1 style="font-size: 1.4rem; font-weight: 700; margin-bottom: 8px; color: var(--text-primary, #f1f5f9);">Something Went Wrong</h1>
                <p style="color: var(--text-secondary, #cbd5e1); margin-bottom: 16px; font-size: 0.95rem; line-height: 1.5;">${this.escapeHtml(errorMessage)}</p>
    ...
    getErrorSummary() {
        ...
        return {
            ...
            types: types,
            recent: log[0] || null,
            lastError: log[0]?.timestamp || null
        };
    },
