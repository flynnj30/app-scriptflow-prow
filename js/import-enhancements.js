// ================================================================
// SMART IMPORT ENHANCEMENTS - RELIABILITY & PERFORMANCE
// ================================================================
...
const ImportQueue = {
    ...
    processRecord: function(record) {
        return new Promise((resolve, reject) => {
            try {
                const validation = validateAppointmentDataEnhanced(record);
    ...
    onComplete: function() {
        ...
        if (failed > 0) {
            setTimeout(() => showImportErrorReport(this.errors), 500);
        }
    }
};
        ...
        for (const recordId of session.recordIds || []) {
            for (let date in AppState.appointments) {
                if (AppState.appointments[date].reports) {
                    const found = AppState.appointments[date].reports.find(r => r.id === recordId);
                    if (found) {
                        Data.deleteAppointment(date, recordId);
                        deletedCount++;
                        break;
                    }
                }
...
const ImportRecovery = {
    ...
    autoSave: function() {
        try {
            const state = {
                records: ImportState.parsedRecords,
                timestamp: new Date().toISOString(),
                totalProcessed: ImportState.totalProcessed,
                totalValid: ImportState.totalValid,
                totalInvalid: ImportState.totalInvalid,
                totalDuplicates: ImportState.totalDuplicates
            };
    ...
    restore: function() {
        ...
        if (confirm(`Recover import from ${new Date(state.timestamp).toLocaleString()}? (${state.records.length} records)`)) {
            ...
            renderImportResults(state.records);
                ...
                ${errors.map((error, index) => `
                    <div class="error-item" style="padding:10px; border:1px solid var(--border-color); border-radius:8px; margin-bottom:8px; border-left:3px solid var(--danger);">
                        ...
                        <div style="font-size:0.8rem; color:var(--danger); margin-top:4px;">
                            ${Array.isArray(error.error?.errors) ? error.error.errors.join('; ') : (error.error || 'Unknown error')}
                        </div>
                        ${error.duplicate ? `
...
function validateAppointmentDataEnhanced(data) {
    ...
    if (data.status) {
        const statusOptions = CONFIG.STATUS_OPTIONS || SMART_IMPORT_CONFIG.VALIDATION.status.allowed;
...
function renderRecordEnhanced(record) {
    ...
    const fieldsHtml = renderRecordFields(record);
                ...
                ${record.hasDuplicate ? `
                    <div class="record-duplicates">
                        ...
                        <ul>${record.duplicates.filter(d => d.confidence >= 60).map(d => 
                            `<li>
                                ${Utils.escapeHtml(d.existing.business)} - ${Utils.escapeHtml(d.existing.contactName)} 
...
function downloadImportTemplate() {
    const template = `Business Name,Contact Name,Phone,Email,Date,Time,Status,Assigned,Notes
ABC Company,John Doe,(555) 123-4567,john@abc.com,2024-07-20,2:30 PM,Warm Callback,Daniel,"High interest, referred by Sarah"
XYZ Corp,Jane Smith,(555) 987-6543,jane@xyz.com,2024-07-21,10:00 AM,Hot Transfer,Sarah,"Decision maker, urgent need"
Tech Solutions,Bob Johnson,(555) 456-7890,bob@tech.com,2024-07-22,3:00 PM,Pending,Mike,"Follow up on proposal"
`;
    ...
    a.href = url;
    a.download = `import_template_${Utils.getTodayStr()}.csv`;
...
function addImportToolbar() {
    const container = DOM.get('action-buttons');
    if (!container) return;
    ...
    const quickImportBtn = DOM.get('quickImportBtn');
    const historyBtn = DOM.get('importHistoryBtn');
    const templateBtn = DOM.get('importTemplateBtn');
    const clipboardBtn = DOM.get('clipboardImportBtn');
    
    if (quickImportBtn) {
        quickImportBtn.addEventListener('click', openSmartImportEnhanced);
    }
