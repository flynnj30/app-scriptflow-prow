...
function showHelpModal() {
    ...
    modal.innerHTML = `<div class="modal-card"><h3><i class="fas fa-question-circle"></i> ScriptFlow Pro Guide</h3>
        ...
        <div style="margin:16px 0;"><strong>📝 Notes Module</strong><br>Rich text editing, markdown, templates, reminders, lock, export/import</div>
...
function showFeaturePanel(featureType, title) {
    ...
    const notepadTabs = document.getElementById('notepadTabContainer');
    ...
    notepadTabs.style.display = 'none';
    ...
    featureTitle.innerHTML = `<i class="fas ${featureType === 'notepad' ? 'fa-sticky-note' : (featureType === 'analytics' ? 'fa-chart-pie' : (featureType === 'calendar' ? 'fa-calendar-alt' : (featureType === 'tasks' ? 'fa-tasks' : 'fa-globe')))}"></i> ${title}`;
    if (featureType === 'notepad') {
        notepadTabs.style.display = 'flex';
        currentView = 'notepad';
        if (window.Notepad) {
            window.Notepad.init();
        } else {
            featureBody.innerHTML = '<div class="loading-spinner"><i class="fas fa-spinner"></i> Loading Notes...</div>';
        }
...
function loadWorkspaceUrl(input, tabId = workspaceState.activeTabId, addToHistory = true) {
    ...
    // Try to load the URL directly in an iframe
        ...
        iframe.onerror = () => {
            viewEl.innerHTML = `
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; background:#fff; color:#333; padding:40px; text-align:center;">
                    <h2 style="color:var(--danger);">Cannot Load</h2>
                    <p style="color:#666;">This page may not allow embedding.</p>
                    <button onclick="window.open('${finalUrl}', '_blank')" style="padding:10px 24px; background:#3b82f6; color:white; border:none; border-radius:8px; cursor:pointer; margin-top:12px;">Open in New Tab</button>
                </div>
            `;
        };
    }
    ...
    setTimeout(() => {
        const overlayEl = document.getElementById('wsLoadingOverlay');
...
function updateCommandResults(query) {
    ...
    const commands = [
        ...
        { label: 'Open Notes', icon: 'fa-sticky-note', action: () => { showFeaturePanel('notepad', '📝 Notes'); }, shortcut: '' }
    ];
                ...
                if (selected) {
                    ...
                    if (results[index]) {
                        const commands = [
                            ...
                            { label: 'Refresh Page', action: () => { const reloadBtn = document.getElementById('wsReloadBtn'); if (reloadBtn) reloadBtn.click(); }},
                            { label: 'Bookmark Page', action: () => { const bookmarkBtn = document.getElementById('wsBookmarkBtn'); if (bookmarkBtn) bookmarkBtn.click(); }},
                            { label: 'Toggle Theme', action: toggleTheme },
                            { label: 'Export Data', action: exportToCSV },
                            { label: 'Help Guide', action: showHelpModal },
                            { label: 'Open Notes', action: () => showFeaturePanel('notepad', '📝 Notes') }
                        ];
        ...
        item.addEventListener('click', (e) => {
            ...
            try {
                if (text.includes('Notes')) {
                    showFeaturePanel('notepad', '📝 Notes');
                } else if (text.includes('Analytics Hub')) {
...
// Initialize Notepad if it exists
if (window.Notepad) {
    window.Notepad.init();
}